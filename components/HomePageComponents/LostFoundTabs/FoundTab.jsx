...
const button_style = "px-4 py-1 text-sm font-medium text-white bg-emerald-800 rounded-full shadow-md hover:bg-emerald-700 transition-all duration-300 h-fit w-fit m-1"
  return (
    <div className={`bg-white/10 backdrop-blur-md rounded-3xl border-2 border-white/20 shadow-xl hover:bg-white/20 hover:shadow-2xl transition-all duration-500 ease-in-out transform hover:scale-[0.99] p-1 text-black font-sans m-1 ${font1.className} w-full flex flex-col animated-gradient-bg-foundtab`}>
        <h2 className="text-5xl font-extrabold mb-3 tracking-wide p-1">Found</h2>
        ...
        <div className=" rounded-3xl h-140 overflow-y-auto">
        {
            posts && Array.isArray(posts) && posts.map((post,index) => (
                <FoundMiniCard key={index} post={post} userPosition={props.userPosition}/>
            ))
        }
        </div>
    </div>
