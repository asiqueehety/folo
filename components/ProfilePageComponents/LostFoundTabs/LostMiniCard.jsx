...
export default function LostMiniCard(props) {
    ...
    const [content_lastused_time, set_content_lastused_time] = useState(null)
    const [content_lastused_date, set_content_lastused_date] = useState(null)
    const [content_finder_reward, set_content_finder_reward] = useState(null)
    ...
    const [edit_content_lastused_time, set_edit_content_lastused_time] = useState(false)
    const [edit_content_lastused_date, set_edit_content_lastused_date] = useState(false)
    ...
    useEffect(() => {
        ...
        set_content_lastused_time(props.post.content_lastused.time)
        set_content_lastused_date(props.post.content_lastused.date)
        set_content_pic(props.post.content_pic)
        set_content_finder_reward(props.post.finder_reward)
  ...
  function editClicked(route) {
    switch (route) {
        ...
        case 'content_details/special':
            ...
            break
        case 'content_lastused/time':
            set_edit_content_lastused_time(true)
            break
        case 'content_lastused/date':
            set_edit_content_lastused_date(true)
            break
  ...
  function saveClicked(route) {
    ...
    else if (route === 'content_lastused/time') value = content_lastused_time;
  ...
  async function edit_in_db(route, value) {
    ...
    try {
        const res = await fetch(`/api/edit/post/lost/${route}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
        ...
        if (res.ok) {
        console.log('Lost post update successful',data)
        } else {
        alert(data.error || 'Lost post update failed')
        }
    } catch (err) {
    ...
    <div className="w-full p-3 bg-neutral-800 text-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 my-1 flex flex-col hover:scale-[0.99]">
        ...
        <div className='flex flex-row'>
            ...
            <div className='flex flex-col ml-2'>
                ...
                <div className='flex flex-row'>
                    ...
                    <button className='bg-yellow-600 text-white p-1 m-0 h-fit w-fit rounded-2xl hover:bg-cyan-950 transition-all text-sm' onClick={()=>{saveClicked('content_name');set_edit_content_name(false);}} disabled={!content_name}>Save</button>
                </div>}
                ...
                <div className='flex flex-row'>
                    <div className="text-sm text-gray-300 mb-2 flex flex-row">
                        ...
                        {!edit_content_type? <Edit className="text-red-500 hover:scale-105 h-4 w-4 m-0.5" onClick={()=>editClicked('content_type')}/>
                        :
                        <div className='flex flex-row'>
                            ...
                            <button className='bg-yellow-600 text-white p-1 m-0 h-fit w-fit rounded-2xl hover:bg-cyan-950 transition-all text-sm' onClick={()=>{saveClicked('content_type');set_edit_content_type(false)}} disabled={!content_type}>Save</button>
                        </div>
                        }</div>
                </div>
        ...
        >
            ...
            >
                ...
                >
                    Color of the item
                    <Edit className="text-red-500 hover:scale-105 h-4 w-4 m-0.5" onClick={()=>editClicked('content_details/color')}/>
                </div>
                :
                <div className='flex flex-row'> 
                    ...
                    <button className='bg-yellow-600 text-white p-1 m-0 h-fit w-fit rounded-2xl hover:bg-cyan-950 transition-all text-sm' onClick={()=>{saveClicked('content_details/color');set_edit_content_details_color(false)}} disabled={!content_details_color}>Save</button>
                </div>
                }
            </div>
            ...
            >
                ...
                <div className='flex flex-row'>
                    ...
                    <button className='bg-yellow-600 text-white p-1 m-0 h-fit w-fit rounded-2xl hover:bg-cyan-950 transition-all text-sm' onClick={()=>{saveClicked('content_details/special');set_edit_content_details_special(false)}} disabled={!content_details_special}>Save</button>
                </div>
                }</div>
            </div>
            <div className={`grid grid-cols-[1fr_3fr] m-2 w-full h-fit p-1 rounded-xl`}
            >
                ...
                <div className='bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-lg text-md p-1 w-fit flex flex-row'>
                    {!edit_content_lastused_time? content_lastused_time:post.content_lastused.time}
                    {!edit_content_lastused_time?
                    <Edit className="text-red-500 hover:scale-105 h-4 w-4 m-0.5" onClick={()=>editClicked('content_lastused/time')}/>
                    :
                    <div className='flex flex-row'>
                    <select 
                        id="dropdown"
                        ...
                        defaultValue={""}
                        onChange={(e)=>{set_content_lastused_time(e.target.value)}}
                        >
                    ...
                    <button className='bg-yellow-600 text-white p-1 m-0 h-fit w-fit rounded-2xl hover:bg-cyan-950 transition-all text-sm' onClick={()=>{saveClicked('content_lastused/time');set_edit_content_lastused_time(false)}} disabled={!content_lastused_time}>Save</button>
                </div>
                    }
                    ,
                    {!edit_content_lastused_date? (formatDate(content_lastused_date)):(formatDate(post.content_lastused.date))}
                    {!edit_content_lastused_date? 
                    <Edit className="text-red-500 hover:scale-105 h-4 w-4 m-0.5" onClick={()=>editClicked('content_lastused/date')}/>
                    :
                    <div className='flex flex-row'>
                    <input type='date' className="peer min-w-30 max-w-46 m-1 placeholder:text-white/30 placeholder:text-xs border border-gray-300 rounded-xl text-sm text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" placeholder='Content Name' onChange={(e)=>{set_content_lastused_date(e.target.value)}} value={content_lastused_date} max={new Date().toISOString().split("T")[0]}></input>
                    <button className='bg-yellow-600 text-white p-1 m-0 h-fit w-fit rounded-2xl hover:bg-cyan-950 transition-all text-sm' onClick={()=>{saveClicked('content_lastused/date');set_edit_content_lastused_date(false)}} disabled={!content_lastused_date}>Save</button>
                </div>
                    }
                </div>
            </div>
            ...
            >
                ...
                <Edit className="text-red-500 hover:scale-105 h-4 w-4 m-0.5" onClick={()=>editClicked('content_location')}/>
            </div>
        </motion.div>
        )}
        </AnimatePresence>
