...
import OwnershipClaimCard from './OwnershipClaimCard'
...
export default function FoundMiniCard(props) {
  ...
  useEffect(() => {
    ...
    async function get_claims() {
        try {
            const response = await fetch(`/api/get_claims/ownership`, {
                method: 'POST',
                headers: {
            ...
            {show_claim && (
                ...
                >
                    {claims.map((claim, index) => (
                        <div key={index}>
                            <OwnershipClaimCard claim={claim} expand_image={expand_image}/>
                        </div>
                    ))}
                </motion.div>
            )}
